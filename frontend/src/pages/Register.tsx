...
import { Shield, Loader, AlertTriangle, CheckCircle2 } from 'lucide-react';

const registerSchema = zod.object({
  email: zod.string().email({ message: 'Enter a valid security email address' }),
  password: zod.string().min(6, { message: 'Secret key must be at least 6 characters' }),
    ...
    <div className="min-h-screen bg-[#080b11] flex items-center justify-center p-4 cyber-grid">
      <div className="w-full max-w-md bg-[#0a0f1d] border border-white/5 rounded-xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl font-bold tracking-wider text-white m-0">REGISTER CREDENTIALS</h1>
          <span className="text-[10px] text-white/40 tracking-widest font-mono uppercase mt-1">ENCRYPT OPERATOR ACCOUNT</span>
        </div>
        ...
        {success && (
          <div className="mb-6 p-4 bg-emerald-950/35 border border-emerald-500/30 rounded-lg flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
            <div className="text-xs text-emerald-300 font-mono">
              Credentials registered successfully. Redirecting to decrypter...
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-mono text-white/50 mb-1.5 uppercase tracking-wider">PASSPHRASE KEY</label>
            <input
              type="password"
              {...register('password')}
              placeholder="••••••••"
          ...
          <div>
            <label className="block text-xs font-mono text-white/50 mb-1.5 uppercase tracking-wider">CONFIRM PASSPHRASE KEY</label>
            <input
              type="password"
            ...
            {errors.confirmPassword && (
              <span className="text-[10px] text-red-400 font-mono mt-1 block">{errors.confirmPassword.message}</span>
            )}
          </div>
        ...
        <div className="mt-8 text-center text-xs text-white/30">
          ...
          <Link to="/login" className="text-[#06b6d4] hover:underline font-mono">
            Portal Access
          </Link>
        </div>
      </div>
