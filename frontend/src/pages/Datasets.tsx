    ...
    <div className="space-y-6 text-left">
        ...
        <div className="xl:col-span-1 space-y-6">
          ...
          <div className="card p-5 space-y-4">
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4 text-xs font-semibold">
              <div
                ...
                onDrop={handleDrop}
                className={`
                  border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center bg-surface-0/30 transition-colors cursor-pointer text-center
                  ${dragActive ? 'border-accent bg-accent-subtle/5' : 'border-border-strong hover:border-accent/40'}
                `}
              >
                <Database className="w-8 h-8 text-text-tertiary mb-2" />
                <span className="text-xs text-text-secondary">Drag & drop CSV file here</span>
                <span className="text-[10px] text-text-tertiary mt-1">Maximum size limit 5MB</span>
                ...
                <label 
                  ...
                  className="mt-4 px-3 py-1.5 border border-border-strong hover:border-accent rounded-lg bg-surface-2 text-text-secondary hover:text-accent text-[10px] uppercase font-bold transition-all cursor-pointer"
                >
              ...
              {uploading && (
                <div className="space-y-1.5">
                  <div className="flex justify-between text-[9px] font-mono-data text-text-tertiary">
          ...
          <div className="card p-5 space-y-4">
            ...
            <form onSubmit={handleStartPreprocessing} className="space-y-4 text-xs font-semibold">
              <div>
                <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Select Ingested File</label>
                <select
              ...
              <div>
                <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Target Label Column</label>
                <input
                  type="text"
                  value={targetColumn}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Missing values</label>
                  <select
                ...
                <div>
                  <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Feature Scaling</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Categorical Encoding</label>
                  <select
                ...
                <div>
                  <label className="block text-[9px] text-text-tertiary mb-1.5 uppercase tracking-wider">Test Split Ratio</label>
                  <input
                    type="number"
                    step="0.05"
                    min="0.05"
            ...
            <div className="p-4 border-b border-border-default flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-surface-1/40">
              ...
              <div className="relative w-full sm:w-44">
                ...
                <input
                  ...
                  className="w-full bg-surface-0 border border-border-strong rounded-lg pl-8 pr-3 py-1 text-[10px] text-white outline-none focus:border-accent"
                />
              </div>
            </div>
            ...
            <div className="p-4 space-y-3 max-h-56 overflow-y-auto">
              {isJobsLoading ? (
                ...
                preprocessingJobs?.map((job) => (
                  <div key={job.id} className="bg-surface-2/60 p-3.5 rounded-lg border border-border-subtle flex flex-col gap-2 text-xs text-left">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        {job.processed_dataset && (
                          <span className="text-[10px] text-accent font-semibold bg-accent-subtle border border-accent-border/30 px-1.5 py-0.5 rounded font-mono-data">
                    ...
                    {job.status === 'failed' && job.error_message && (
                      <div className="text-[10px] text-semantic-critical bg-semantic-critical/5 border border-semantic-critical/10 p-2 rounded flex items-start gap-1 font-mono-data">
                ...
                {isProfileLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-text-secondary">
                    <div className="space-y-4">
                      <div>
                        <span className="text-[9px] text-text-tertiary uppercase font-bold tracking-wider">Duplicate Record Rows</span>
                        <div className="text-sm font-bold text-white mt-0.5 font-mono-data">{datasetProfile.duplicate_rows} rows</div>
                      </div>
                      <div>
                        <span className="text-[9px] text-text-tertiary uppercase font-bold tracking-wider">Numeric Features ({datasetProfile.numeric_features?.length || 0})</span>
                        <div className="text-text-secondary font-mono-data mt-1.5 max-h-24 overflow-y-auto border border-border-strong p-2.5 bg-surface-0 rounded-lg text-[10px]">
                    ...
                    <div className="space-y-4">
                      <div>
                        <span className="text-[9px] text-text-tertiary uppercase font-bold tracking-wider">Target label distributions</span>
                        <div className="mt-1.5 space-y-2 bg-surface-0 border border-border-subtle p-3 rounded-lg">
      ...
      {deleteTargetId && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface-1 border border-border-strong rounded-xl max-w-md w-full p-6 shadow-2xl space-y-4 text-left">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={deleteMutation.isPending}
                className="px-4 py-2 bg-semantic-critical hover:bg-red-600 disabled:opacity-40 text-[#02040a] font-bold rounded-lg text-xs transition-colors flex items-center gap-2 cursor-pointer"
              >
